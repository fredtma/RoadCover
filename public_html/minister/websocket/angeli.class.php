...
// FIXME throw error if message length is longer than 0x7FFFFFFFFFFFFFFF chracters
   function send($client,$data,$all=false){
    $this->say("> ".$data);
    ...
    if ($all) {foreach($this->users as $user){if($user->socket!=$client) {$result = socket_write($user->socket, $header . $data, strlen($data) + $header_length);}}}
    else {$result = socket_write($client, $header . $data, strlen($data) + $header_length);}
