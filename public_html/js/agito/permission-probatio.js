...
if($('#frm_permission').data('iota'))$('.frm_permission_default').hide();
$('#frm_permission').submit(function(){setTimeout(function(){
   if($('#frm_permission').data('iota')){$('.frm_permission_default').show();}
},100)});
$('.frm_permission_default button').click(function(){
   var n = $('#page').val()+' '+$(this).val();
   var d = 'Permission to '+$(this).val()+' '+$('#page').val();
   if(!$(this).data('toggle')||$(this).data('toggle')==0){$(this).data('toggle',1);theForm.newTableRow({"name":n,"desc":d},'permissions',true);}
